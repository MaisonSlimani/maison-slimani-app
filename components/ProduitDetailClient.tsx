...
// import PWAProduitContent from '@/app/boutique/[categorie]/[slug]/PWAProduitContent'
...
// Re-defining interfaces if they are not exported from types (based on page.tsx)
// Using local interfaces if @/types doesn't have them all or matches exactly
...
interface LocalTaille {
    nom: string
    stock: number
}

interface LocalCouleur {
    nom: string
    code?: string
    images?: string[]
    stock?: number
...
interface LocalProduit {
    ...
    stock: number
    image_url?: string
    images?: LocalImageItem[] | string[]
    couleurs?: LocalCouleur[]
    has_colors?: boolean
    categorie: string
    vedette: boolean
    date_ajout: string
...
export default function ProduitDetailClient({ produitInitial }: ProduitDetailClientProps) {
    ...
    // const searchParams = useSearchParams() // Not needed for ID fetch anymore
    const router = useRouter()
    const { addItem, items, clearCart } = useCart()
    const [produit, setProduit] = useState<LocalProduit | null>(produitInitial)
    // const [loading, setLoading] = useState(true) // Data is passed as prop
    ...
    // Removed generateSlug helper as it is not used in the client component logic we kept

    // Removed loading effect with fetch logic since we receive data as props
    ...
    useEffect(() => {
        ...
        const channel = supabase
            ...
            .on(
                ...
                (payload) => {
                    const updated = payload.new as any // casting to any to avoid strict type mismatch with local interface
    ...
    useEffect(() => {
        ...
        // Document title and meta tags handling is now Server Side, excluding structured data which is also best on server but can stay here if needed.
        // However, for consistency, we'll remove the document.title and meta tag manipulation in useEffect as it invalidates the purpose of Server Components for SEO.
        // Structured data (JSON-LD) can be added via metadata in server component or kept here if dynamic update is needed.
        // Keeping JSON-LD here for now as it uses refs/dom
        ...
        if (produit) {
            const imageUrl = produit.image_url || (produit.images && Array.isArray(produit.images) && produit.images.length > 0
                ? (typeof produit.images[0] === 'string' ? produit.images[0] : (produit.images[0] as LocalImageItem).url)
                : '')
    ...
    // Render desktop/responsive version
    return (
