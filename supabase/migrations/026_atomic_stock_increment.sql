-- Atomic stock increment functions to restore stock when orders are canceled
-- These functions increment stock atomically in a single transaction

-- Function for products without colors
CREATE OR REPLACE FUNCTION incrementer_stock_atomic(
  produit_id UUID,
  quantite INTEGER
)
RETURNS BOOLEAN AS $$
