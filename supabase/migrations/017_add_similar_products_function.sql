...
BEGIN
  ...
  WHERE 
    ...
    AND (
      ...
      OR (
        -- Product without colors: must have stock > 0
        (NOT p.has_colors AND p.stock > 0)
        OR
        -- Product with colors: at least one color must have stock > 0
        (p.has_colors AND p.couleurs IS NOT NULL AND EXISTS (
          SELECT 1 
          FROM jsonb_array_elements(p.couleurs) AS couleur
          WHERE (couleur->>'stock')::INTEGER > 0
        ))
