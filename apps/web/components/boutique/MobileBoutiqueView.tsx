...
export default function MobileBoutiqueView({ initialCategorie = 'tous', initialSearch = '' }: MobileBoutiqueViewProps) {
  ...
  return (
    <div className="w-full min-h-screen pb-20 bg-background">
      <BoutiqueHeader searchQuery={searchQuery} setSearchQuery={setSearchQuery} onFocus={() => setIsSearchOverlayOpen(true)} categorie={categorie} categories={categories} router={router} />
        ...
        <div className="px-4 py-8">
          {loadingCategories ? <CategoryCardSkeleton count={4} /> : (
            <div className="space-y-6">
              ...
              <div className="grid gap-6">{categoriesWithImages.map((cat, idx) => <motion.div key={cat.titre} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.1 }}><CarteCategorie {...cat} priority={idx < 2} /></motion.div>)}</div>
              <div className="text-center pt-8 border-t border-border/50"><h2 className="text-xl font-serif mb-4">Voir tous nos produits</h2><Button asChild className="bg-dore text-charbon hover:bg-dore/90 px-8"><Link href="/boutique/tous">Découvrir la collection</Link></Button></div>
            </div>
          )}
        </div>
      ) : (
        <div className="px-4 py-6">
          {loadingProducts ? <div className="grid grid-cols-2 gap-4"><ProductCardSkeleton count={6} /></div> : produits.length === 0 ? (
            ...
            <div className="grid grid-cols-2 gap-4">{produits.map((p, idx) => <motion.div key={p.id} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.05 }}><ProductCard produit={p} priority={idx === 0} /></motion.div>)}</div>
          )}
        </div>
      )}
