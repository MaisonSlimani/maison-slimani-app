...
export function BoutiqueHeader({ searchQuery, setSearchQuery, onFocus, category, categories, router }: BoutiqueHeaderProps) {
  ...
  return (
    <header className="sticky top-0 z-40 bg-background/95 backdrop-blur-md border-b border-border safe-area-top">
      ...
      {(category !== 'tous' || searchQuery) && !!categories.length && (
        <div className="px-4 py-2 border-b border-border/50">
          <div className="flex gap-2 overflow-x-auto scrollbar-hide pb-1">
            <button onClick={() => router.push('/boutique?categorie=tous')} className={cn("flex-shrink-0 px-4 py-1.5 rounded-full text-sm font-medium whitespace-nowrap", category === 'tous' ? "bg-dore text-charbon" : "bg-muted text-muted-foreground")}>Tous</button>
            {categories.map((cat: { name: string; slug: string }) => (
              <button key={cat.slug} onClick={() => router.push(`/boutique?categorie=${cat.slug}`)} className={cn("flex-shrink-0 px-4 py-1.5 rounded-full text-sm font-medium whitespace-nowrap", category === cat.slug ? "bg-dore text-charbon" : "bg-muted text-muted-foreground")}>{cat.name}</button>
            ))}
          </div>
        </div>
      )}
