...
export function MobileHomeCollections({ categories }: { categories: CategoryCardItem[] }) {
  return (
    <section className="py-12 px-4">
      <h2 className="text-2xl font-serif text-center mb-8">Collections</h2>
      <div className="grid grid-cols-2 gap-4">
        {categories.map((cat, i: number) => (
          <motion.div key={cat.link} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}>
            <Link href={cat.link} className="block group" onClick={() => hapticFeedback('light')}>
              ...
              <h3 className="font-serif text-center text-sm">{cat.title}</h3>
            </Link>
          </motion.div>
        ))}
      </div>
