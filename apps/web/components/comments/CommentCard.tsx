...
export const CommentCard = ({
  ...
  return (
    <div className="border border-border rounded-lg p-4 space-y-3 bg-card">
      <div className="flex items-start justify-between">
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-1">
            <h4 className="font-semibold font-serif text-foreground">{comment.nom}</h4>
          ...
          <p className="text-xs text-muted-foreground">
            ...
            {comment.updated_at && comment.created_at !== comment.updated_at && <span className="ml-2">(modifié)</span>}
          </p>
        </div>
      ...
      <p className="text-sm text-foreground/80 leading-relaxed whitespace-pre-wrap">{comment.commentaire}</p>
        ...
        <div className={cn("grid gap-2 mt-3", comment.images.length >= 3 ? "grid-cols-3" : `grid-cols-${comment.images.length}`)}>
          {comment.images.map((img: string, i: number) => (
            <div key={i} className="relative aspect-square rounded-lg overflow-hidden border border-border group cursor-pointer" onClick={() => window.open(img, '_blank')}>
              <OptimizedImage src={img} alt="Comment" fill objectFit="cover" className="transition-transform group-hover:scale-105" />
            </div>
          ))}
        </div>
      )}
