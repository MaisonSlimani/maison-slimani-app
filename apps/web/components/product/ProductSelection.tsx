...
interface Variation { name: string; code?: string; stock?: number }
      ...
      {hasColors && (
        <div className="space-y-4">
          ...
          <div className="flex gap-4">
            {colors.map((c) => (
              <button key={c.name} onClick={() => setSelectedColor(c.name)} className={cn("group relative flex flex-col items-center gap-2 transition-all p-0.5 rounded-lg border", selectedColor === c.name ? "border-charbon scale-110" : "border-transparent")}>
                <div className="w-12 h-12 rounded-md shadow-inner ring-1 ring-charbon/5" style={{ backgroundColor: c.code || '#000' }} />
                <span className="text-[9px] font-medium absolute -bottom-5 opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">{c.name}</span>
              </button>
            ))}
          </div>
        </div>
    ...
    <div className="flex gap-4 pt-4">
      <Button 
        ...
        className={cn(
          ...
          isInCart ? "bg-charbon text-white" : "bg-dore text-charbon hover:bg-dore/90"
        )} 
