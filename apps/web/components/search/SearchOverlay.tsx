...
interface SearchOverlayProps { isOpen: boolean; onClose: () => void; basePath?: string }

interface SimpleProduct { id: string; nom: string; price: number; image_url?: string; slug: string; categorie?: string }
...
export default function SearchOverlay({ isOpen, onClose, basePath = '' }: SearchOverlayProps) {
  ...
  const { data: results, isLoading } = useQuery<SimpleProduct[]>({
    ...
    queryFn: async () => {
      ...
      const resp = await fetch(`/api/produits?search=${encodeURIComponent(debouncedQuery)}&limit=8`)
              ...
              {debouncedQuery.trim() ? (
                ...
                (results && results.length > 0) ? (
                  <div className="space-y-4">
                    ...
                    {results.map((p) => <SearchResultItem key={p.id} product={{ nom: p.nom, image_url: p.image_url, prix: p.price }} productHref={`${basePath}/boutique/${slugify(p.categorie || 'v')}/${p.slug}`} isActive={false} onClick={onClose} />)}
                  </div>
