...
function VariationHeader({ variation, onUpdate, onRemove }: { variation: VariationWithPending; onUpdate: (u: VariationWithPending) => void; onRemove: () => void }) {
  return (
      ...
      <div className="grid grid-cols-2 gap-4 flex-1">
        ...
        <div className="space-y-2"><Label>Code Hex</Label>
          <div className="flex gap-2">
            <Input type="color" value={variation.code || '#000000'} onChange={e => onUpdate({ ...variation, code: e.target.value })} className="w-12 p-1 h-10" />
            <Input value={variation.code || ''} onChange={e => onUpdate({ ...variation, code: e.target.value })} placeholder="#000000" />
          </div>
        </div>
      </div>
      <Button variant="ghost" size="icon" onClick={onRemove} className="text-red-600 hover:bg-red-50 ml-2"><Trash2 className="w-4 h-4" /></Button>
    </div>
  )
}
  ...
  return (
    <Card className="p-4 bg-muted/20 border-dashed">
      ...
      <div className="space-y-4">
        <TailleGrid variation={variation} onUpdate={u => onChange(index, u)} />
        <div><Label className="text-xs uppercase tracking-wider text-muted-foreground">Images</Label>
          <div className="flex flex-wrap gap-2 mt-2">
            {(variation.images || []).map((url, i) => (
              <div key={i} className="relative w-16 h-16 rounded border overflow-hidden group"><img src={url} alt="" className="w-full h-full object-cover" />
                <button type="button" onClick={() => onChange(index, { ...variation, images: variation.images?.filter((_, idx) => idx !== i) })} className="absolute top-0 right-0 bg-red-600 text-white p-0.5 opacity-0 group-hover:opacity-100"><X className="w-3 h-3" /></button>
              </div>
            ))}
            {(variation.pendingImages || []).map((img, i) => (
              <div key={`p-${i}`} className="relative w-16 h-16 rounded border overflow-hidden bg-dore/5 border-dore/30"><img src={img.url} alt="" className="w-full h-full object-cover" />
                <button type="button" onClick={() => onChange(index, { ...variation, pendingImages: variation.pendingImages?.filter((_, idx) => idx !== i) })} className="absolute top-0 right-0 bg-charbon text-white p-0.5"><X className="w-3 h-3" /></button>
              </div>
            ))}
            <label className="w-16 h-16 rounded border-2 border-dashed flex items-center justify-center cursor-pointer hover:bg-muted/50"><ImageIcon className="w-4 h-4 text-muted-foreground" /><input type="file" multiple accept="image/*" onChange={hI} className="hidden" /></label>
          </div>
        </div>
      </div>
    </Card>
