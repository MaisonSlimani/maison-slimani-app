...
export function ProductListItem({ product, onEdit, onDeleteRequest }: ProductListItemProps) {
  const stockTotal = product.has_colors && product.couleurs && Array.isArray(product.couleurs)
    ? product.couleurs.reduce((sum, c) => sum + (c.stock || 0), 0)
    : product.stock || 0
      ...
      <div className="flex gap-4">
        ...
        <div className="flex-1 min-w-0 flex flex-col justify-between">
          <div>
            ...
            <h3 className="font-medium mb-1 truncate text-base leading-tight">{product.nom}</h3>
            <p className="text-sm text-primary font-serif font-medium mb-2">
              {product.prix.toLocaleString('fr-MA')} DH
            </p>

            <div className="mb-3">
              <StockStatus stock={stockTotal} />
              {product.has_colors && product.couleurs && Array.isArray(product.couleurs) && product.couleurs.length > 0 && (
                <div className="mt-2 space-y-1">
                  {product.couleurs.slice(0, 3).map((c, idx) => (
                  ...
                  {product.couleurs.length > 3 && (
                    <p className="text-[10px] text-muted-foreground italic">+{product.couleurs.length - 3} autres...</p>
                  )}
                </div>
              )}
            </div>
          ...
          <div className="flex gap-2 justify-end mt-2">
            ...
            <Button variant="ghost" size="icon" onClick={() => onDeleteRequest(product.id)} className="h-8 w-8 text-red-600 hover:text-red-700 hover:bg-red-50">
