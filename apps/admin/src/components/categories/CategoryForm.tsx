...
interface CategoryFormData { name: string; slug: string; description: string; image_url: string; order: number; isActive: boolean }
  ...
  return (
    <form onSubmit={(e) => handleLocalSubmit(e)} className="space-y-6">
      <div className="grid grid-cols-2 gap-4">
        <div><Label>Nom *</Label>
          <Input required value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value, slug: editing ? formData.slug : generateSlug(e.target.value) })} />
        </div>
      ...
      <div className="flex items-center gap-2"><input type="checkbox" checked={formData.isActive} onChange={e => setFormData({ ...formData, isActive: e.target.checked })} className="w-4 h-4" /><Label className="cursor-pointer">Catégorie active</Label></div>
