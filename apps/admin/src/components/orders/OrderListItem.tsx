        ...
        <div className="flex gap-2 flex-shrink-0">
          {(commande.items ?? []).slice(0, 3).map((p, idx) => (
            <div key={idx} className="relative w-16 h-16 rounded-lg overflow-hidden border border-border bg-muted">
              {p.image_url ? <img src={p.image_url} alt="" className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-[10px]">{p.name.substring(0, 2)}</div>}
            </div>
          ))}
          {(commande.items ?? []).length > 3 && <div className="w-16 h-16 rounded-lg border border-border bg-muted flex items-center justify-center text-xs font-medium text-muted-foreground">+{(commande.items ?? []).length - 3}</div>}
        </div>
        <div className="flex-1 flex justify-between items-start">
          <div className="flex-1">
            <div className="flex items-center gap-4 mb-3">
              <h3 className="text-xl font-serif">Commande #{commande.id.substring(0, 8)}</h3>
              <span className={`px-3 py-1 rounded-full text-xs font-medium ${getStatutColor(commande.status)}`}>{commande.status}</span>
              <span className="text-sm text-muted-foreground">{commande.orderedAt ? new Date(commande.orderedAt).toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', hour: '2-digit', minute: '2-digit' }) : ''}</span>
            </div>
            <div className="mb-4 flex flex-wrap gap-1">{(commande.items ?? []).map((p, idx) => (<span key={idx} className="text-xs bg-muted px-2 py-0.5 rounded">{p.name} ×{p.quantity}</span>))}</div>
            <div className="grid md:grid-cols-3 gap-4 text-sm">
              <div><p className="text-muted-foreground">Client</p><p className="font-medium">{commande.customerName}</p></div>
              <div><p className="text-muted-foreground">Téléphone</p><p className="font-medium">{commande.phone}</p></div>
